...
export function startClient(env){
  ...
  let ws,serial=0,reconnectTimer,shuttingDown=false,connected=false,deviceMenuOpen=false,selectedRoot="",selectedArtworkURL="",parents=[],libraryEntries=[],libraryParent="",libraryCursor="",queueRenderKey="",queueFocus=null,reloaded=sessionStorage.getItem("go2tv-protocol-reload")==="1";
  ...
  function renderQueue(){
    ...
    const selectedItem=items.find(item=>item.selected);if(queueFocus&&selectedItem?.id!==queueFocus.previousSelectedID){queueFocus=null;selectedRow?.scrollIntoView({behavior:"smooth",block:"nearest"})}
  }
  ...
  function selectLibraryMedia(item){const previousSelectedID=state.queue.find(queued=>queued.selected)?.id||"";state.selected_media=true;state.selected_media_name=item.name;state.artwork_id="";selectedArtworkURL=item.artwork_url||"";renderPlayback();const requestID=send("library.select_media",{root_id:selectedRoot,entry_id:item.id});queueFocus=requestID?{requestID,previousSelectedID}:null}
