...
export function startClient(env) {
  ...
  const status = byID("status"),
    ...
    deviceTrigger = byID("device-trigger"),
  ...
  const icon = (name, spin = false) => {
    const node = document.createElementNS(svgNS, "svg"),
      use = document.createElementNS(svgNS, "use");
    node.setAttribute("class", `action-icon${spin ? " is-spinning" : ""}`);
  ...
  const setButtonIcon = (node, name, label, spin = false) => {
    if (
      node.dataset.icon !== name ||
      node.dataset.iconSpinning !== String(spin)
    ) {
      node.replaceChildren(icon(name, spin));
      node.dataset.icon = name;
      node.dataset.iconSpinning = String(spin);
    }
    node.title = label;
  ...
  const button = (label, action, options = {}) => {
    ...
    node.title = options.title ?? (options.icon ? label : "");
    node.ariaLabel = options.ariaLabel || node.ariaLabel || "";
    node.addEventListener("click", action);
    return node;
  };
  const actionGroup = (...nodes) => {
  ...
  const playbackState = () =>
    String(state.playback_state || "STOPPED").toUpperCase();
  const hasPending = (type, itemID = "") =>
    [...pending.values()].some(
      (request) =>
        request?.type === type &&
  ...
  const normalizeImageDuration = (value) => {
  ...
  let themeChoice = localStorage.getItem("go2tv-theme");
  if (!themeOrder.includes(themeChoice)) themeChoice = "auto";
  setButtonIcon(byID("stop-button"), "square", "Stop");
  ...
  function closeArtwork() {
    ...
    byID("artwork-modal-image").removeAttribute("src");
    if (modal.open) modal.close();
  }
  function openArtwork(item) {
    const modal = byID("artwork-modal"),
      image = byID("artwork-modal-image");
    text("artwork-modal-title", item.name);
    image.alt = `Artwork for ${item.name}`;
    image.hidden = false;
  ...
  function mediaThumbnail(item) {
    ...
    thumbnail.ariaLabel = `View artwork for ${item.name}`;
    thumbnail.title = "View artwork";
    image.alt = "";
    image.loading = "lazy";
    image.decoding = "async";
    image.src = item.thumbnail_url;
    fallback.className = "thumbnail-fallback";
    fallback.textContent = mediaGlyph(item.media_kind);
    ...
    thumbnail.addEventListener("click", () => openArtwork(item));
  ...
  function renderDevices() {
    const selected = state.selected_device_id || "",
      items = state.devices || [],
      selectedDevice = items.find((device) => device.id === selected),
      disabled = !connected || shuttingDown || hasPending("devices.select");
    deviceTrigger.replaceChildren();
    deviceTrigger.dataset.selected = String(!!selectedDevice);
    deviceTrigger.ariaExpanded = String(deviceMenuOpen);
    deviceTrigger.disabled = disabled || !items.length;
    if (selectedDevice) appendDeviceContent(deviceTrigger, selectedDevice);
    else {
      ...
      label.textContent = items.length
        ? "Choose a renderer"
        : "No renderers found";
      deviceTrigger.append(label);
    }
    devices.replaceChildren();
    devices.hidden = !deviceMenuOpen;
    for (const device of items) {
      const row = document.createElement("button");
      row.type = "button";
      row.className = "device-option";
      row.dataset.selected = String(device.id === selected);
      row.role = "option";
      row.ariaSelected = String(device.id === selected);
      row.disabled = disabled;
      row.addEventListener("click", () => {
        deviceMenuOpen = false;
        send("devices.select", { device_id: device.id });
      });
      appendDeviceContent(row, device);
      devices.append(row);
    }
    byID("refresh").disabled =
      !connected || shuttingDown || hasPending("devices.refresh");
  }
  function appendDeviceContent(node, device) {
    const name = document.createElement("span"),
      badges = document.createElement("span"),
      protocol = String(device.protocol || "Renderer");
    name.className = "device-name";
    name.textContent = device.label;
    name.title = device.label;
    badges.className = "device-badges";
    badges.append(deviceBadge(protocol, protocol.toLowerCase()));
    if ((device.capabilities || []).includes("audio_only"))
      badges.append(deviceBadge("Audio only", "audio-only"));
    node.append(name, badges);
  }
  function deviceBadge(label, kind) {
  ...
  function queueAction(item, locked) {
    const current = playbackState(),
      starting =
        (item.selected && current === "LOADING") ||
        hasPending("player.play", item.id);
    if (starting)
      return {
        label: "Starting…",
        icon: "loader-circle",
    ...
    if (item.active && current === "PLAYING")
      return {
        label: "Pause",
        icon: "pause",
        disabled: locked,
        run: () => send("player.pause"),
      };
    if (item.active && current === "PAUSED")
      return {
        label: "Resume",
        icon: "play",
        disabled: locked,
        run: () => send("player.resume"),
      };
    if (item.active && current === "STOPPING")
      return {
        label: "Stopping…",
        icon: "loader-circle",
        spin: true,
    ...
    return {
      ...
      run: () => send("player.play", { item_id: item.id }),
    };
  }
  const queueRows = () =>
    [...queue.children].filter((node) => node.className === "queue-row");
  function finishQueueDrag(pointerID, commit) {
    if (
      !queueDrag ||
      (pointerID !== undefined && queueDrag.pointerID !== pointerID)
    )
      return;
    ...
    try {
      if (drag.control.hasPointerCapture?.(drag.pointerID))
        drag.control.releasePointerCapture(drag.pointerID);
    } catch {}
    if (commit && drag.toIndex !== drag.fromIndex && !queueLocked())
      send("queue.move", {
        item_id: drag.itemID,
        delta: drag.toIndex - drag.fromIndex,
      });
  }
  function updateQueueDrag(event) {
    ...
    for (const [index, row] of rows.entries()) {
      delete row.dataset.dropPosition;
      if (index === target && target !== queueDrag.fromIndex)
        row.dataset.dropPosition =
          target < queueDrag.fromIndex ? "before" : "after";
    }
    const bounds = queue.getBoundingClientRect(),
      edge = Math.min(48, bounds.height / 4);
    if (event.clientY < bounds.top + edge)
      queue.scrollBy?.({ top: -16, behavior: "auto" });
    else if (event.clientY > bounds.bottom - edge)
      queue.scrollBy?.({ top: 16, behavior: "auto" });
  }
  function queueDragControl(item, index, locked, row, count) {
    const control = document.createElement("button"),
      label = `Reorder ${item.name || "Untitled media"}`;
    control.type = "button";
    control.className = "queue-drag-handle icon-action";
    control.disabled = !connected || locked || count < 2;
    setButtonIcon(control, "grip-vertical", `${label}. Drag or use arrow keys`);
    control.title = "Drag to reorder";
    control.setAttribute("aria-keyshortcuts", "ArrowUp ArrowDown");
    control.addEventListener("pointerdown", (event) => {
      ...
      queue.dataset.dragging = "true";
      control.setPointerCapture?.(event.pointerId);
    });
    control.addEventListener("pointermove", updateQueueDrag);
    control.addEventListener("pointerup", (event) => {
      event.preventDefault();
      finishQueueDrag(event.pointerId, true);
    });
    control.addEventListener("pointercancel", (event) =>
      finishQueueDrag(event.pointerId, false),
    );
    ...
    control.addEventListener("keydown", (event) => {
      ...
      if (
        ...
        return;
      event.preventDefault();
      send("queue.move", { item_id: item.id, delta });
    });
    return control;
  }
  function renderQueue() {
    const items = state.queue || [],
      locked = queueLocked(),
      starting = [...pending.values()]
        .filter((request) => request?.type === "player.play")
        .map((request) => request.payload?.item_id ?? "");
    ...
    text("queue-count", String(items.length));
    if (!items.length) {
      const empty = document.createElement("li");
      empty.className = "empty-state";
      empty.textContent = "Playlist is empty. Add something from your library.";
      queue.append(empty);
      return;
    }
    let currentRow = null;
    for (const [index, item] of items.entries()) {
      ...
      if (item.selected) row.dataset.current = "true";
      ...
      label.textContent = item.name || "Untitled media";
      ...
      meta.textContent = item.active
        ...
        : item.selected
          ? "Current"
          : item.parent || kindLabel(item.kind);
      copy.append(meta);
      ...
      const actions = actionGroup(
        button(primary.label, primary.run, {
          ...
          className: "queue-primary icon-action",
          icon: primary.icon,
          spin: primary.spin,
          title: primary.label,
          ariaLabel: `${primary.label.replace("…", "")} ${item.name}`,
        }),
        queueDragControl(item, index, locked, row, items.length),
        button("Remove", () => send("queue.remove", { item_id: item.id }), {
          disabled: locked || removeProtected,
          className: "remove-action icon-action",
          icon: "trash-2",
          title: removeTitle,
    ...
    if (queueFocus && currentItem?.id !== queueFocus.previousCurrentID) {
      queueFocus = null;
  ...
  function renderPlayback() {
    ...
    const volumePending = hasPending("player.volume"),
      ...
      mute = byID("mute"),
      muteLabel = state.muted ? "Unmute" : "Mute";
    byID("volume-down").disabled = !canControlVolume || volumePending;
    byID("volume-up").disabled = !canControlVolume || volumePending;
    ...
    let command = "player.play",
      label = "Play",
      disabled =
        !state.selected_media && !state.queue?.some((item) => item.selected);
    if (current === "PLAYING") {
    ...
    play.dataset.command = command;
    setButtonIcon(
      play,
      working ? "loader-circle" : current === "PLAYING" ? "pause" : "play",
      label,
      working,
    );
    play.disabled =
      !connected || shuttingDown || disabled || hasPending(command);
    stop.disabled =
      !connected ||
      shuttingDown ||
      (!state.has_session && current !== "LOADING") ||
      current === "STOPPING" ||
      hasPending("player.stop");
    const art = byID("artwork"),
    ...
    switch (message.type) {
      ...
      case "state.selection":
        ...
        Object.assign(state, {
          ...
          selected_media: p.media ?? state.selected_media,
          selected_media_name: p.media_name ?? state.selected_media_name,
          selected_subtitle: p.subtitle ?? state.selected_subtitle,
          selected_subtitle_name:
            p.subtitle_name ?? state.selected_subtitle_name,
          transcode: p.transcode ?? state.transcode,
          media_type: p.media_type ?? state.media_type,
          artwork_id: p.artwork_id ?? state.artwork_id,
        });
        renderDevices();
        renderPlayback();
        renderPolicy();
      ...
      case "state.policy":
        state.revision = p.revision ?? state.revision;
        state.policy = p.policy || state.policy;
        renderPolicy();
        break;
      case "pending":
      ...
      case "error": {
        ...
        if (
          p.code === "conflict" &&
          request &&
          request.attempt < maxConflictRetries
        ) {
          const retryID = send(
            request.type,
            request.payload,
  ...
  function send(type, payload = {}, attempt = 0) {
    ...
    ws.send(
      JSON.stringify({
        ...
        type,
        id,
        payload: { ...clean, expected_revision: state.revision },
      }),
    );
    return id;
  }
  ...
  function renderLibrary() {
    ...
    for (const item of entries) {
      ...
      if (item.thumbnail_url) main.append(mediaThumbnail(item));
        ...
        icon.className =
          item.kind === "directory" ? "entry-icon folder-icon" : "entry-icon";
        icon.ariaHidden = "true";
        if (item.kind !== "directory") icon.textContent = "CC";
        main.append(icon);
      }
      main.append(copy);
      row.append(main);
      if (item.kind === "directory")
        row.append(
          actionGroup(
            button(
              "Open",
              () => {
                parents.push({ id: item.id, name: item.name });
                browse(item.id);
              },
        ...
        row.append(
          actionGroup(
            button("Play", () => playLibraryMedia(item), {
            ...
            button(
              "Add to playlist",
              () =>
                send("queue.add", { root_id: selectedRoot, entry_id: item.id }),
              {
                className: "icon-action",
                icon: "list-plus",
                title: "Add to playlist",
                ariaLabel: `Add ${item.name} to playlist`,
              },
            ),
          ),
        );
  ...
  function renderLoadMore() {
    ...
    const more = button("Load more", () => {
      more.disabled = true;
      browse(libraryParent, libraryCursor, true);
    });
    nav.append(more);
    library.append(nav);
  }
  async function browse(parentID = "", cursor = "", append = false) {
    const query = new URLSearchParams({ root_id: selectedRoot, limit: "200" });
    ...
    try {
      ...
      libraryParent = parentID;
      libraryCursor = data.cursor || "";
      renderBreadcrumbs();
      renderLibrary();
    } catch (error) {
  ...
  function sendPolicy(changed = "") {
    ...
    const auto = byID("autoplay").checked,
      duration = normalizeImageDuration(byID("image-duration").value);
    byID("image-duration").value = String(duration);
    send("playback.policy", {
      policy: {
  ...
  document.addEventListener("click", (event) => {
    if (deviceMenuOpen && !event.composedPath().includes(devicePicker)) {
      deviceMenuOpen = false;
      renderDevices();
    }
  });
  document.addEventListener("keydown", (event) => {
    if (deviceMenuOpen && event.key === "Escape") {
      deviceMenuOpen = false;
      renderDevices();
      deviceTrigger.focus();
    }
  ...
  byID("volume-up").addEventListener("click", () =>
    send("player.volume", { delta: 1 }),
  );
  byID("mute").addEventListener("click", () =>
    send("player.mute", { muted: !state.muted }),
  );
  byID("transcode").addEventListener("change", (event) =>
    send("player.transcode", { enabled: event.target.checked }),
  );
  byID("subtitle-clear").addEventListener("click", () =>
    send("library.clear_subtitle"),
  );
  byID("library-filter").addEventListener("input", renderLibrary);
  byID("artwork").addEventListener("error", () => {
    byID("artwork").hidden = true;
    byID("artwork-placeholder").hidden = false;
  });
  byID("artwork-modal-image").addEventListener("error", () => {
    showToast("Artwork unavailable", "error");
    closeArtwork();
  });
  byID("artwork-modal-close").addEventListener("click", closeArtwork);
  ...
  byID("theme-toggle").addEventListener("click", () => {
    themeChoice =
      themeOrder[(themeOrder.indexOf(themeChoice) + 1) % themeOrder.length];
    localStorage.setItem("go2tv-theme", themeChoice);
    applyTheme();
  });
