...
func TestArtworkPlaybackRowKeepsSquareAndSpace(t *testing.T) {
	...
	playback := container.NewVBox(widget.NewSlider(0, 100), widget.NewButton("Cast", nil))
	...
	for _, width := range []float32{row.MinSize().Width, 640, 900} {
		...
		if playback.Size().Height != art.Size().Height || playback.Position().Y != art.Position().Y {
			t.Fatal("artwork and control surfaces have unequal vertical bounds")
		}
