...
### New Package: Cast Protocol

#### [NEW] castprotocol/client.go

Core Chromecast client wrapper with full implementations:
...
// If startTime > 0, seeks to that position after loading.
// If subtitleURL is non-empty, loads WebVTT subtitles.
func (c *CastClient) Load(mediaURL string, contentType string, startTime int, subtitleURL string) error {
    c.mu.Lock()
    defer c.mu.Unlock()

    // Build load options
    opts := []application.LoadOption{}
    if subtitleURL != "" {
        opts = append(opts, application.WithSubtitles(subtitleURL))
    }

    // Load with options if available, otherwise standard load
    if len(opts) > 0 {
        if err := c.app.LoadWithOptions(mediaURL, contentType, opts...); err != nil {
            return err
        }
    } else {
        if err := c.app.Load(mediaURL, contentType, false, false, false); err != nil {
            return err
        }
    }
    ...
    if startTime > 0 {
        if err := c.app.SeekFromStart(startTime); err != nil {
            // Non-fatal: log but don't fail load
            return nil
        }
    }
...
#### [NEW] castprotocol/status.go

Status types for Chromecast:
...
type CastStatus struct {
    ...
    CurrentTime     float64 // Current position in seconds
