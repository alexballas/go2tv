...
func TestSetAVTransportSoapBuild(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			contentFeatures, err := utils.BuildContentFeatures(tc.mediaType, seekflag, tc.transcode)
			if err != nil {
			...
			out, err := setAVTransportSoapBuild(tc.mediaURL, tc.mediaType, tc.subtitleURL, tc.transcode, tc.seek)
			if err != nil {
