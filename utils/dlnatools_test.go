...
func TestGetMimeDetailsFromPath(t *testing.T) {
	tDir := t.TempDir()
	p := filepath.Join(tDir, "video.mp4")
	if err := os.WriteFile(p, videoBytes, 0o644); err != nil {
		t.Fatalf("TestGetMimeDetailsFromPath setup failed with error %s", err.Error())
	}

	_, err := GetMimeDetailsFromPath(p)
	if err != nil {
		t.Fatalf("TestGetMimeDetailsFromPath failed with error %s", err.Error())
	}
}
...
func TestBuildContentFeatures(t *testing.T) {
	tt := []struct {
		...
		},
		{
			...
			`00`,
			"DLNA.ORG_PN=AVC_TS_MP_HD_AAC_MULT5;DLNA.ORG_OP=00;DLNA.ORG_CI=0;DLNA.ORG_FLAGS=01700000000000000000000000000000",
			false,
		},
	}
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			out, err := BuildContentFeatures(tc.mediaType, tc.seek, tc.transcode)
			if err != nil {
